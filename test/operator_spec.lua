...
describe('operator-pending mode', function()
  ...
  it('dge at BOL wraps to previous line deleting previous word (CJK)', function()
  ...
  it('dge at BOL with ASCII deletes previous word and joins', function()
    helpers.api.nvim_buf_set_lines(0, 0, -1, false, { 'hello world', 'foo bar' })
    helpers.api.nvim_win_set_cursor(0, { 2, 0 })
    helpers.feed('dge')
    local lines = helpers.api.nvim_buf_get_lines(0, 0, -1, false)
    eq(1, #lines)
    eq('hello foo bar', lines[1])
  end)

  it('de at EOL wraps to next line (CJK)', function()
    -- ICU merges "世界" into one token, so de deletes the entire
    -- merged run plus the newline.
